...
def clean_smfluo1():
    ...
    """
    with open("../../../data/raw/smfluo1_db/smfluo1_raw.csv", "r") as csvfile_raw:
        csv_reader = csv.reader(csvfile_raw)
        header = next(csv_reader)
        with open("../../../data/intermediate/smfluo1_db/smfluo1_clean.csv", "w") as csvfile_clean:
            csv_writer = csv.writer(csvfile_clean)
            csv_writer.writerow(["compound", "solvent", "lambda_max"])
            for row in csv_reader:
                csv_writer.writerow([row[0], row[1], float(row[2])])
        csvfile_clean.close()
    csvfile_raw.close()
