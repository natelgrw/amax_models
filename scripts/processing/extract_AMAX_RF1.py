...
for the AMAX Random Forest model. Generates publication-ready figures and tables.
"""
...
def load_rf_model():
    ...
    model = joblib.load('models/AMAX_RF1.pkl')
    return model

def evaluate_model(model, X, y):
    """Evaluate Random Forest model and return predictions"""
...
def create_prediction_vs_actual_plot(y_true, y_pred, title, filename):
    ...
    # create scatter plot
    plt.scatter(y_true, y_pred, alpha=0.6, s=20, color='forestgreen')
    ...
    plt.tight_layout()
    plt.savefig(f'performance/AMAX_RF1/{filename}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_residual_plot(y_true, y_pred, title, filename):
    ...
    ax1.scatter(y_pred, residuals, alpha=0.6, s=20, color='forestgreen')
    ...
    ax2.hist(residuals, bins=50, alpha=0.7, color='forestgreen', edgecolor='black')
...
def create_error_distribution_plot(y_true, y_pred, title, filename):
    ...
    # create histogram
    plt.hist(errors, bins=50, alpha=0.7, color='forestgreen', edgecolor='black')
...
def create_feature_importance_plot(model, feature_names, X_train, y_train, title, filename, top_n=20):
    ...
    bars = plt.barh(y_pos, top_importance, color='forestgreen', alpha=0.7)
...
def main():
    """Main function to extract all publication data for Random Forest model"""
    print("AMAX Random Forest Model - Publication Data Extraction")
    print("=" * 60)
    
    # create performance directories
    os.makedirs('performance', exist_ok=True)
    os.makedirs('performance/AMAX_RF1', exist_ok=True)
    ...
    create_prediction_vs_actual_plot(y_val, val_pred, "AMAX_RF1 - Validation Set", "validation/pred_vs_actual")
    create_prediction_vs_actual_plot(y_test, test_pred, "AMAX_RF1 - Test Set", "testing/pred_vs_actual")
    
    # 2. residual plots
    create_residual_plot(y_val, val_pred, "AMAX_RF1 - Validation Set", "validation/residuals")
    create_residual_plot(y_test, test_pred, "AMAX_RF1 - Test Set", "testing/residuals")
    
    # 3. error distribution plots
    create_error_distribution_plot(y_val, val_pred, "AMAX_RF1 - Validation Set", "validation/error_dist")
    create_error_distribution_plot(y_test, test_pred, "AMAX_RF1 - Test Set", "testing/error_dist")
    
    # 4. feature importance plot
    print("\nGenerating feature importance plot...")
    top_features, top_importance = create_feature_importance_plot(model, X_train.columns, X_train, y_train, "AMAX_RF1", "feature_importance", top_n=20)
    ...
    val_predictions_df.to_csv('performance/AMAX_RF1/validation/val_predictions.csv', index=False)
    ...
    test_predictions_df.to_csv('performance/AMAX_RF1/testing/test_predictions.csv', index=False)
    
    print("\n" + "="*50)
    print("AMAX_RF1 PUBLICATION DATA EXTRACTION COMPLETE")
    print("="*50)
